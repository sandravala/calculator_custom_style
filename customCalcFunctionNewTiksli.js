...
function rodytiLaukusIsmokosSkaiciavimui(ismoka) {
	switch(ismoka) {
		...
		case 'mamosDU' :
			...
			label[mamosPajamuLaukas].text('Mamos vidutinis darbo užmokestis prieš mokesčius');
		...
		case 'tecioDU' :
			...
			label[tecioPajamuLaukas].text('Tėčio vidutinis darbo užmokestis prieš mokesčius');
...
function createRow(data, ismokuPavadinimas) {
    ...
    for(let i = 0; i < data.length ; i++) {
    			rows += `<tr>
					...
					<td style='text-align: left; font-size: .85em; padding-left: .3em; font-weight: ${fontWeight};'><div class='bendra-suma' style='font-size: 1em;'>${data[i].suma}</div></td>
					<td style='text-align: left; font-size: .85em; padding-left: .3em; font-weight: ${fontWeight};'><div class='bendra-suma' style='font-size: 1em;'>${data[i].sumaPoMokesciu}</div></td>
